...
public class StepIndicatorView: UIView {
    ...
    public var displayPreviousFinishedSteps: Bool = false
    
    @IBInspectable public var displayCenterCircle: Bool = false {
        didSet {
            self.setDisplayCenter()
        }
    }
    ...
    private func setDisplayCenter() {
        for i in 0..<self.numberOfSteps {
            self.annularLayers[i].displayCenterCircle = displayCenterCircle
        }
    }
...
//                    if displayPreviousFinishedSteps {
